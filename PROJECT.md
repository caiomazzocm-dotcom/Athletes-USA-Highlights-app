...
31. Studio usability pass (Sept 2026):
    ...
    - **Timeline persistence.** The Studio autosaves the edit to `localStorage` under `STORAGE_KEY_PROJECT` (`aua_studio_project`), debounced 600ms, and restores it on entry. Header shows a "Saved HH:MM" indicator plus a **New** button; a banner after restore offers "Start fresh".
